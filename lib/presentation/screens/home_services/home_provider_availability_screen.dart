...
import '../../bloc/home_service_availability/home_service_availability_cubit.dart';
import '../../bloc/home_service_availability/home_service_availability_state.dart';

class HomeProviderAvailabilityScreen extends StatefulWidget {
  const HomeProviderAvailabilityScreen({super.key});

  @override
  State<HomeProviderAvailabilityScreen> createState() =>
      _HomeProviderAvailabilityScreenState();
}
...
class _HomeProviderAvailabilityScreenState
    extends State<HomeProviderAvailabilityScreen> {
  final _cubit = sl<HomeServiceAvailabilityCubit>();
  ...
  void _onState(BuildContext context, HomeServiceAvailabilityState state) {
    if (state is HomeServiceAvailabilityLoaded && !_initialized) {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await sl<HomeServiceAvailabilityRepository>().replaceMyAvailability(
        slots,
      );
      await sl<HomeServiceAvailabilityRepository>().replaceMyExceptions(
        exceptions,
      );
      await _cubit.load();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:
          BlocConsumer<
            HomeServiceAvailabilityCubit,
            HomeServiceAvailabilityState
          >(
            bloc: _cubit,
            listener: _onState,
            builder: (context, state) {
              if (state is HomeServiceAvailabilityLoading && !_initialized) {
                return const Center(child: CircularProgressIndicator());
              }
              if (state is HomeServiceAvailabilityError && !_initialized) {
                return Center(
                  child: Padding(
