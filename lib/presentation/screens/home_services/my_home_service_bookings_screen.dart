  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: BlocConsumer<MyHomeServiceBookingsCubit, MyHomeServiceBookingsState>(
          ...
          builder: (context, state) {
            ...
            if (state is MyHomeServiceBookingsLoaded) {
              return TabBarView(
                children: [
                  _BookingList(bookings: state.pending, emptyMessage: 'No pending bookings.', isPerformingAction: state.isPerformingAction, canCancel: true),
                  _BookingList(bookings: state.upcoming, emptyMessage: 'No upcoming bookings.', isPerformingAction: state.isPerformingAction, canCancel: true),
                  _BookingList(bookings: state.history, emptyMessage: 'No past bookings.', isPerformingAction: state.isPerformingAction, canCancel: false),
                ],
              );
            }
          ...
          ? ListView(
              children: [
                SizedBox(
                  ...
                  child: Center(child: Text(emptyMessage, style: const TextStyle(color: Colors.grey, fontSize: 14))),
                ),
              ],
            )
          : ListView.builder(
...
class _BookingCard extends StatelessWidget {
  const _BookingCard({required this.booking, required this.isPerformingAction, required this.canCancel});
  ...
  Widget build(BuildContext context) {
    ...
    final price = booking.totalPrice != null ? '\$${booking.totalPrice!.toStringAsFixed(2)}' : null;
            ...
            if (booking.serviceAddress != null && booking.serviceAddress!.isNotEmpty) ...[
              ...
              Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 6),
                  Expanded(child: Text(booking.serviceAddress!, style: const TextStyle(fontSize: 12, color: Color(0xFF3A80C2)))),
                ],
              ),
            ],
            if (canCancel &&
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  child: isPerformingAction
                      ? const SizedBox(height: 16, width: 16, child: CircularProgressIndicator(strokeWidth: 2, color: _orange))
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.12), borderRadius: AppRadius.radius20),
