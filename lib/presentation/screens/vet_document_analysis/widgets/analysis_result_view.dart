...
class AnalysisResultView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _card(child: Text(result.summary, style: const TextStyle(fontSize: 14, color: _textDark, height: 1.5))),
        ...
        if (result.abnormalValues.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          ...result.abnormalValues.map((v) => Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (v.referenceRange != null) ...[
                        ...
                        Text('Reference range: ${v.referenceRange}',
                            style: const TextStyle(fontSize: 12, color: _textMuted)),
                      ],
                      const SizedBox(height: 6),
                      Text(v.interpretation, style: const TextStyle(fontSize: 13, color: _textDark)),
                    ],
                  ),
                ),
              )),
          ...
          children: items
              .map((item) => Padding(
                    ...
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('•  ', style: TextStyle(color: _textDark, fontWeight: FontWeight.bold)),
                        Expanded(
                          child: Text(item, style: const TextStyle(fontSize: 14, color: _textDark, height: 1.4)),
                        ),
                      ],
                    ),
                  ))
  ...
  Widget _disclaimer(String text) => Container(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Icon(Icons.info_outline_rounded, size: 18, color: Color(0xFFB07D00)),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
                text,
                style: const TextStyle(fontSize: 12.5, color: Color(0xFF6B4E00), height: 1.4),
              ),
            ),
          ],
        ),
