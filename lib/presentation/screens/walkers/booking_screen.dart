    ...
    if (state is WalkBookingSubmitting) {
      return Center(
        child: Column(
          ...
          children: [
            const CircularProgressIndicator(color: AppColors.navWalkers),
    ...
    if (photoUrl != null && photoUrl!.isNotEmpty) {
      return ClipOval(
        child: Image.network(
          ...
          height: size,
          fit: BoxFit.cover,
          errorBuilder: (_, _, _) => _fallback(size, cs),
        ),
      );
    }
    return _fallback(size, cs);
  }

  Widget _fallback(double s, ColorScheme cs) => Container(
        width: s,
        height: s,
        ...
        child: Icon(Icons.pets_rounded, size: 22, color: cs.onSurfaceVariant),
      );
}
