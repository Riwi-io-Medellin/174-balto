...
class _MockRemoteDataSource extends Mock implements VetDocumentAnalysisRemoteDataSource {}
...
void main() {
  ...
  test('returns the result from the remote datasource on success', () async {
    ...
    when(() => remote.analyze(context: any(named: 'context'), fileUrls: any(named: 'fileUrls')))
        .thenAnswer((_) async => result);

    final actual = await repository.analyze(context: context, fileUrls: fileUrls);
  ...
  test('wraps a DioException as a NETWORK_ERROR failure', () async {
    when(() => remote.analyze(context: any(named: 'context'), fileUrls: any(named: 'fileUrls'))).thenThrow(
      DioException(requestOptions: RequestOptions(path: '/vet-document-analysis'), message: 'timeout'),
    );

    await expectLater(
      () => repository.analyze(context: context, fileUrls: fileUrls),
      throwsA(isA<VetDocumentAnalysisFailure>().having((e) => e.code, 'code', 'NETWORK_ERROR')),
    );
  });
}
