...
void main() {
  ...
  test(
    'parses a successful response into a VetDocumentAnalysisResult',
    () async {
      when(
        () => dio.post<dynamic>(
          any(),
  ...
  test(
    'maps a 400 error body to a VetDocumentAnalysisFailure with the server code',
    () async {
      when(
        () => dio.post<dynamic>(
          any(),
  ...
  test('maps a 503 with no structured body to AI_UNAVAILABLE', () async {
    ...
    ).thenAnswer((_) async => _response(503, null));
  ...
  test('maps a 502 with no structured body to AI_PARSE_ERROR', () async {
    ...
    ).thenAnswer((_) async => _response(502, null));
